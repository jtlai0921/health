...
import com.jtlai0921.pass.graph.MyMarkerView;
...
public class graph extends AppCompatActivity {
    ...
    private void setChart() {
        ...
        xAxis.setLabelCount(6,true);//設定X軸上要有幾個標籤
        chart.getDescription().setEnabled(true);//讓右下角文字消失
//        xAxis.setEnabled(false);//去掉X軸數值
        xAxis.setDrawGridLines(true);//將X軸格子消失掉
        ...
        LineDataSet set1 = new LineDataSet(yValues1, "BMI");
        LineDataSet set2 = new LineDataSet(yValues2, "血糖");
    ...
    private void setChartImage(LineDataSet set,int i){//設置圖表線
        ...
        dataText.setDrawValues(true);//不要再點上顯示值
    }

    private static class MyValueFormatter extends ValueFormatter{//設置Ｘ軸
        @Override
        public String getFormattedValue(float value) {
            long millis = TimeUnit.SECONDS.toMillis((long) value);
            return String.format(String.valueOf(new Date(millis)));
        }
    }
